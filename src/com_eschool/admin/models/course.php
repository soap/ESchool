 ...
 * @since       1.0
 */
class EschoolModelCourse extends JModelAdmin
{
	/**
	...
	{
		$condition = array(
			'course_group_id = '.(int) $table->course_group_id
		);
	 ...
	 * @since   1.0
	 */
	public function getTable($type = 'Course', $prefix = 'EschoolTable', $config = array())
	{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (empty($table->ordering)) {
				...
				$query->from('#__eschool_courses');
				$query->where('course_group_id = '.(int) $table->course_group_id);
