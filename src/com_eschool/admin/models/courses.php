 ...
 * @since       1.0
 */
class EschoolModelCourses extends JModelList
{
	/**
	...
	protected function populateState($ordering = 'title', $direction = 'asc')
	{
		...
		$value = $app->getUserStateFromRequest($this->context.'.filter.coursegroup_id', 'filter_coursegroup_id');
		$this->setState('filter.coursegroup_id', $value);
	...
	protected function getListQuery()
	{
		...
		$query->from('#__eschool_courses AS a');
		...
		$coursegroupId = $this->getState('filter.coursegroup_id');
		if (is_numeric($coursegroupId)) {
			$query->where('a.course_group_id = '.(int) $coursegroupId);
		}
		else if (is_array($coursegroupId)) {
			JArrayHelper::toInteger($coursegroupId);
			$coursegroupId = implode(',', $coursegroupId);
