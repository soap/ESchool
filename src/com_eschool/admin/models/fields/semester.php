...
class JFormFieldSemester extends JFormField
{
	...
	protected function getInput()
	{
		...
		$title = ($this->value ? $this->getSemesterTitle() : JText::_('COM_ESCHOOL_SELECT_A_SEMESTER'));
	...
	{
		$html = array();
		$link = 'index.php?option=com_eschool&amp;view=semesters'
				. '&amp;layout=modal&amp;tmpl=component'
						. '&amp;func=esSelectSemester_' . $this->id;
		...
		if ($this->element['readonly'] != 'true') {
			...
			$html[] = '<a class="modal_' . $this->id . '" title="' . JText::_('COM_ESCHOOL_SELECT_SEMESTER') . '"'
					. ' href="' . $link . '" rel="{handler: \'iframe\', size: {x: 800, y: 500}}">';
			$html[] = JText::_('COM_ESCHOOL_SELECT_SEMESTER') . '</a>';
			$html[] = '    </div>';
			$html[] = '</div>';
	...
	protected function getSiteHTML($title)
	{
		...
		. '&amp;layout=modal&amp;tmpl=component&amp;function=pfSelectSemester_' . $this->id;
		...
		if ($this->element['readonly'] != 'true') {
			...
			$html[] = '<a class="modal_' . $this->id . ' btn" title="' . JText::_('COM_ESCHOOL_SELECT_SEMESTER') . '"'
	...
	protected function getJavascript()
	{
		...
		$script[] = 'function esSelectSemester_' . $this->id . '(id, title)';
		$script[] = '{';
	 ...
	 */
	protected function getSemesterTitle()
	{
		$default = JText::_('COM_ESCHOOL_SELECT_A_SEMESTER');
