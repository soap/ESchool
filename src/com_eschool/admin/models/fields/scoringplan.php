 ...
 *
*/
class JFormFieldScoringplan extends JFormField
{
	/**
	 * The form field type.
	 *
	...
	protected function getInput()
	{
		...
		$title = ($this->value ? $this->getScoringPlanTitle() : JText::_('COM_ESCHOOL_SELECT_A_SCORING_PLAN'));
	...
	{
		$html = array();
		$link = 'index.php?option=com_eschool&amp;view=scoringplans'
				. '&amp;layout=modal&amp;tmpl=component'
						. '&amp;func=esSelectScoringPlan_' . $this->id;
		...
		if ($this->element['readonly'] != 'true') {
			...
			$html[] = '<a class="modal_' . $this->id . '" title="' . JText::_('COM_ESCHOOL_SELECT_SCORING_PLAN') . '"'
					. ' href="' . $link . '" rel="{handler: \'iframe\', size: {x: 800, y: 500}}">';
			$html[] = JText::_('COM_ESCHOOL_SELECT_SCORING_PLAN') . '</a>';
			$html[] = '    </div>';
			$html[] = '</div>';
	...
	protected function getSiteHTML($title)
	{
		...
		. '&amp;layout=modal&amp;tmpl=component&amp;func=esSelectScoringPlan_' . $this->id;
		...
		if ($this->element['readonly'] != 'true') {
			...
			$html[] = '<a class="modal_' . $this->id . ' btn" title="' . JText::_('COM_ESCHOOL_SELECT_SCORING_PLAN') . '"'
	...
	protected function getJavascript()
	{
		...
		$script[] = 'function esSelectScoringPlan_' . $this->id . '(id, title)';
		$script[] = '{';
	 ...
	 */
	protected function getScoringPlanTitle()
	{
		$default = JText::_('COM_ESCHOOL_SELECT_A_SCORING_PLAN');
		...
		$query->select('a.id')
		->from('#__eschool_scoring_plans AS a')
		->select('se.title as semester_title')
		->join('LEFT', '#__eschool_semesters AS se ON se.id=a.semester_id')
		->select('c.title as course_title')
		->join('LEFT', '#__eschool_courses as c ON c.id=a.syllabus_course_id')
		...
		return $row->semester_title.' /'.$row->course_title;
	}
}