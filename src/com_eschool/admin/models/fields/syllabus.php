 ...
 *
*/
class JFormFieldSyllabus extends JFormField
{
	/**
	 * The form field type.
	 *
	...
	protected function getInput()
	{
		...
		$title = ($this->value ? $this->getSyllabusTitle() : JText::_('COM_ESCHOOL_SELECT_A_SYLLABUS'));
	...
	protected function getAdminHTML($title)
	{
		...
		$link = 'index.php?option=com_eschool&amp;view=syllabuses'
				...
				. '&amp;func=esSelectSyllabus_' . $this->id;
		...
		if ($this->element['readonly'] != 'true') {
			...
			$html[] = '<a class="modal_' . $this->id . '" title="' . JText::_('COM_ESCHOOL_SELECT_SYLLABUS') . '"'
					. ' href="' . $link . '" rel="{handler: \'iframe\', size: {x: 800, y: 500}}">';
			$html[] = JText::_('COM_ESCHOOL_SELECT_SYLLABUS') . '</a>';
			$html[] = '    </div>';
			$html[] = '</div>';
	...
	protected function getSiteHTML($title)
	{
		...
		. '&amp;layout=modal&amp;tmpl=component&amp;function=pfSelectSyllabus_' . $this->id;
		...
		if ($this->element['readonly'] != 'true') {
			...
			$html[] = '<a class="modal_' . $this->id . ' btn" title="' . JText::_('COM_ESCHOOL_SELECT_SYLLABUS') . '"'
	...
	protected function getJavascript()
	{
		...
		$script[] = 'function esSelectSyllabus_' . $this->id . '(id, title)';
		$script[] = '{';
	...
	{
		$default = JText::_('COM_ESCHOOL_SELECT_A_SYLLABUS');
		...
		->from('#__eschool_syllabuses AS a')
