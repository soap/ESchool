...
class JFormFieldExam extends JFormField
{
	...
	protected function getInput()
	{
		...
		$title = ($this->value ? $this->getSyllabusTitle() : JText::_('COM_ESCHOOL_SELECT_AN_EXAM'));
	...
	{
		$html = array();
		$link = 'index.php?option=com_eschool&amp;view=exams'
				. '&amp;layout=modal&amp;tmpl=component'
				. '&amp;func=esSelectExam_' . $this->id;
		...
		if ($this->element['readonly'] != 'true') {
			...
			$html[] = '<a class="modal_' . $this->id . '" title="' . JText::_('COM_ESCHOOL_SELECT_EXAM') . '"'
					. ' href="' . $link . '" rel="{handler: \'iframe\', size: {x: 800, y: 500}}">';
			$html[] = JText::_('COM_ESCHOOL_SELECT_EXAM') . '</a>';
			$html[] = '    </div>';
			$html[] = '</div>';
	...
	protected function getSiteHTML($title)
	{
		...
		$link = PFticketsHelperRoute::getExamsRoute()
		. '&amp;layout=modal&amp;tmpl=component&amp;function=pfSelectExam_' . $this->id;
		...
		if ($this->element['readonly'] != 'true') {
			...
			$html[] = '<a class="modal_' . $this->id . ' btn" title="' . JText::_('COM_ESCHOOL_SELECT_EXAM') . '"'
	...
	protected function getJavascript()
	{
		...
		$script[] = 'function esSelectExam_' . $this->id . '(id, title)';
		$script[] = '{';
	 ...
	 */
	protected function getSyllabusTitle()
	{
		$default = JText::_('COM_ESCHOOL_SELECT_AN_EXAM');
