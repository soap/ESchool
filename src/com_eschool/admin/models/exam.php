 ...
 * @since       1.0
 */
class EschoolModelExam extends JModelAdmin
{
	/**
	 * Method to get the Exam form.
	 *
	...
	{
		$condition = array(
			'scoring_plan_id = '.(int) $table->scoring_plan_id
		);
	 ...
	 * @since   1.0
	 */
	public function getTable($type = 'Exam', $prefix = 'EschoolTable', $config = array())
	{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (empty($table->ordering)) {
				...
				$query->from('#__eschool_exams');
				$query->where('scoring_plan_id = '.(int) $table->scoring_plan_id);
